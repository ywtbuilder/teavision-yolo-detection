...
# ======================= YOLO-TBD 茶叶检测改进模块 =======================
# 基于论文: YOLO-TBD: Tea Bud Detection with Triple-Branch Attention Mechanism
# and Self-Correction Group Convolution (Industrial Crops & Products 226, 2025)


class TBAM(nn.Module):
    """Triple-Branch Attention Mechanism (三分支注意力机制).
    
    根据 YOLO-TBD 论文 Section 3.2 实现:
    - 分支1: H 维度的跨维度交互 (通道池化 + 1×1卷积 + Sigmoid)
    - 分支2: W 维度的跨维度交互 (通道池化 + 1×1卷积 + Sigmoid)
    - 分支3: 基于能量函数的像素相似度计算 (类似 SimAM)
    
    特点: 无额外参数增加，仅使用 2 个 1×1 卷积核
    
    Args:
        channels (int): 输入通道数
        lmbda (float): 能量函数的正则化参数，默认 1e-4 (论文最优值)
    """
...
class SCGC(nn.Module):
    """Self-Correction Group Convolution (自校正分组卷积).
    
    根据 YOLO-TBD 论文 Section 3.3 实现:
    - 第一部分: 自校正分支 (下采样 → 分组卷积 → 上采样 → Sigmoid门控)
    - 第二部分: 1×1 卷积直接映射分支
    
    目的: 扩大感受野，建立长距离空间依赖，减少误检和漏检
    ...
    def __init__(self, c1: int, c2: int, k: int = 3, s: int = 1, r: int = 2):
        ...
        self.k2 = nn.Sequential(
            nn.AvgPool2d(kernel_size=r, stride=r),  # 下采样
            nn.Conv2d(c1, c1, kernel_size=k, padding=k // 2, groups=c1, bias=False),  # 分组深度卷积
            nn.BatchNorm2d(c1),
            nn.SiLU(),
        )
        # 上采样将在 forward 中使用 F.interpolate 实现（支持任意输入尺寸）
...
class BottleneckSCGC(nn.Module):
    ...
    将标准 Bottleneck 中的 3×3 卷积替换为 SCGC 模块，
    并在输出后接入 TBAM 注意力机制。
...
class C2fTBAM(nn.Module):
    ...
    基于 YOLO-TBD 论文 Figure 5 实现:
    - 使用 BottleneckSCGC 替换标准 Bottleneck
    - 在 C2f 输出后添加 TBAM 注意力
...
# ======================= BiFormer 双层路由注意力模块 =======================
# 基于论文: BiFormer: Vision Transformer with Bi-Level Routing Attention
# (CVPR 2023)


class BiLevelRoutingAttention(nn.Module):
    """Bi-Level Routing Attention (双层路由注意力机制).
    
    根据 BiFormer 论文 Section 3.2 实现:
    - 第一层: 区域级路由 (Region-to-Region Routing)
      - 将特征图划分为 S×S 个区域
      - 计算区域间亲和力图，保留 top-k 最相关区域
    - 第二层: Token级注意力 (Token-to-Token Attention)
      - 在路由后的区域内进行细粒度注意力计算
      - 使用 gather 操作实现 GPU 友好的密集矩阵乘法
    
    复杂度: O((HW)^{4/3}) vs 全局注意力 O((HW)^2)
    
    Args:
        dim (int): 输入通道数
        num_heads (int): 注意力头数，默认 8
        n_win (int): 区域划分数 S，特征图被划分为 S×S 个区域，默认 7
        topk (int): 每个区域保留的最相关区域数，默认 4
        qk_scale (float): Q·K 缩放因子，默认 None (使用 head_dim^-0.5)
        side_dwconv (int): LCE 局部增强的深度卷积核大小，默认 5
    """
    ...
    def __init__(
        ...
        # LCE: Local Context Enhancement (局部上下文增强)
        # 使用深度卷积提供局部归纳偏置
        self.lce = nn.Conv2d(
            dim, dim, 
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 将特征图划分为区域: (B, C, H, W) -> (B, S^2, HW/S^2, C)
        ...
        q, k, v = qkv[0], qkv[1], qkv[2]  # 各 (B, heads, S^2, HW/S^2, head_dim)
        
        # ============ 第三步: 区域级路由 ============
        # 计算区域级 query 和 key (通过区域内平均)
        ...
        # Top-k 路由: 每个区域选择 topk 个最相关的区域
        topk = min(self.topk, num_regions)  # 确保 topk 不超过区域总数
        _, topk_indices = torch.topk(region_affinity, k=topk, dim=-1)  # (B, heads, S^2, topk)
        
        # ============ 第四步: Gather 键值对 ============
        # 根据路由索引收集 K 和 V
        # topk_indices: (B, heads, S^2, topk) -> 扩展用于 gather
        
        # 将 k, v 重排以便 gather: (B, heads, S^2, tokens_per_region, head_dim)
        # 需要 gather 的维度是 S^2 (dim=2)
        
        # 扩展索引以匹配 tokens 和 head_dim
        ...
        # 合并 topk 区域的 tokens: (B, heads, S^2, topk*tpr, head_dim)
        ...
        # 恢复空间形状: (B, S^2, tpr, C) -> (B, C, H_pad, W_pad)
...
class C2fBRA(nn.Module):
    ...
    结合 C2f 结构 + BiFormer 的 BRA 注意力:
    - 使用 Bottleneck 进行特征提取
    - 在输出前应用 BRA 进行全局上下文建模
    
    特别适合小目标检测，因为 BRA 的稀疏采样保留了精细细节。
    ...
    def __init__(
        ...
        self.bra = BiLevelRoutingAttention(
            dim=c2,
            num_heads=max(1, c2 // 32),  # 每个 head 32 通道
            n_win=n_win,
            topk=topk
        )
...
# ==============================================================================
# 方案 D: SPD-Conv + CARAFE 小目标优化模块
# ==============================================================================

class SPDConv(nn.Module):
    """
    SPD-Conv: Space-to-Depth Convolution (空间到深度卷积)
    
    来源: "No More Strided Convolutions or Pooling: A New CNN Building Block 
           for Low-Resolution Images and Small Objects" (CVPR 2022 相关研究)
    
    核心思想:
    - 传统下采样 (stride=2 或 pooling) 会直接丢弃 3/4 的像素信息
    - SPD 将空间信息"折叠"到通道维度，实现 0 信息损失的下采样
    - 对小目标检测尤其有效
    
    工作原理:
    - 输入: (B, C, H, W)
    - Space-to-Depth: (B, C, H, W) -> (B, 4C, H/2, W/2)
    - 1x1 Conv 压缩通道: (B, 4C, H/2, W/2) -> (B, C2, H/2, W/2)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        将 2x2 的空间块折叠到通道维度，保留所有像素信息
        """
        # 获取输入尺寸
        B, C, H, W = x.shape
        
        # Space-to-Depth: 将 2x2 空间块变成 4 个通道
        # x[..., ::2, ::2] 取偶数行偶数列 (左上)
        # x[..., 1::2, ::2] 取奇数行偶数列 (左下)
        # x[..., ::2, 1::2] 取偶数行奇数列 (右上)
        # x[..., 1::2, 1::2] 取奇数行奇数列 (右下)
        x = torch.cat([
            x[..., ::2, ::2],    # 左上角像素
            x[..., 1::2, ::2],   # 左下角像素
            x[..., ::2, 1::2],   # 右上角像素
...
class CARAFE(nn.Module):
    """
    CARAFE: Content-Aware ReAssembly of FEatures (内容感知特征重组)
    
    来源: "CARAFE: Content-Aware ReAssembly of FEatures" (ICCV 2019)
    
    核心思想:
    - 传统上采样 (双线性插值、最近邻) 是"死板"的，不考虑图像内容
    - CARAFE 根据输入内容动态生成上采样核，实现"内容感知"上采样
    - 对小目标特征的还原尤其有效
...
class C2fSPD(nn.Module):
    """
    C2fSPD: C2f 模块 + SPD-Conv 下采样
    
    专门设计用于替换 backbone 中的 Conv(stride=2) 下采样层
    结合 C2f 的跨阶段特征融合能力和 SPD 的无损下采样
